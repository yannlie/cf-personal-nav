...
async function getMe(request, env) {
  if (isPublicMode(env)) {
    return json({ username: 'public', publicMode: true });
  }
...
async function getConfig(env) {
  return json({ publicMode: isPublicMode(env) });
}
...
async function getSites(request, env) {
  const username = isPublicMode(env) ? PUBLIC_USER : await currentUser(request, env);
  if (!username) return json({ error: '未登录' }, 401);
